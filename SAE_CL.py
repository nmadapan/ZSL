'''
	...
	known as Joint Semantic Encoder (JSE) which is an extension to
	the SAE (Semantic Auto Encoder) approach proposed by Kodriov et al.
	JSE is also referred to as SAE_CL (SAE with classification loss). 

	The class, SAE_CL is implemented in a format similar to 
	...
	Notations:
		...
		* W (d x a) - Weight matrix

	Closed form solution using SAE_CL:
		* $$ P W + W Q = R $$
		* $$ P = X^T X, Q = (SS SS^T + \\beta I )(\\alpha S^T S + \\gamma I)^{-1} $$
		* $$ R = (\\alpha X^T Y S + (1+\\gamma)X^T SS^T)(\\alpha S^T S + \\gamma I)^{-1}$$
		* P (d x d), Q (a x a), R (d x a), W (d x a)
...
class SAE_CL(BaseEstimator):
	def __init__(self, alphap = 5e5, gammap = 5e5, betap = 1e2, degree = None, rs = None, debug = False):
		'''
		Description:
			* This class inherits BaseEstimator which defines 
				get_params() and set_params() functions. 
		Input parameters:
			alphap: Hyper parameter. Coefficient of ESZSL loss. 
			betap: Hyper parameter. Regularization coefficient. 
			gammap: Hyper parameter. SAE loss coefficient (not the reconstruction loss coeff).
	...
	def fit(self, X, S, y):
		...
		# B = self.alphap * np.dot(K.T, K) # d x d
		# C = (1+self.alphap) * np.dot(S[y, :].T, K) # a x d

		## Fixed - Used this for the PAA Journal paper. 
		U = np.linalg.pinv(self.alphap * np.dot(S.T, S) + self.gammap * np.eye(S.shape[1])) # a x a
		Sigma = S[y, :]
		YY = np.eye(S.shape[0])[y, :]
		J = np.dot(Sigma.T, Sigma) + self.betap * np.eye(S.shape[1])

		# ########### Regular SAE CL V1 ###############
		# A = np.dot(K.T, K) # d x d
		# B = self.betap * U # a x a 
		# T = np.dot(np.dot(K.T, YY), S) + self.alphap * np.dot(K.T, Sigma)
		# C = np.dot(T, U) # d x a

		########### Regular SAE CL V2 ###############
		A = np.dot(K.T, K) # d x d
		B = np.dot(J, U) # a x a 
		T = np.dot(self.alphap * np.dot(K.T, YY), S) + (1+self.gammap) * np.dot(K.T, Sigma)
		C = np.dot(T, U) # d x a

		W = solve_sylvester(A, B, C) # d x a 
		self.W_ = W
		self.S_ = S
		return self
...
if __name__ == '__main__':
	...
	for mat_fpath in mat_files:
		...
		for iter_idx in range(K):
			...
			clf = ZSLPipeline([('s', UnitScaler()),
							   ('c', SAE_CL(degree = None)),
							  ])
			clf.set_params(c__alphap = 5e5, c__betap = 1e4)
			clf.fit(X_tr, S_tr, Y_tr)
			# S_ts = normalize(S_ts, axis = 0)
